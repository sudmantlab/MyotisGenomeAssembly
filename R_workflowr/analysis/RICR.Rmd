...
tr.upham <- read.tree('../data/stableTraits/UphamEtAl2019_newChiroptera.tree')
...
life.anc <- read_tsv('../data/stableTraits/UphamEtAl2019_newChiroptera_lifespan.ancstates') %>% 
...
tr.life <- read.tree('../data/stableTraits/UphamEtAl2019_newChiroptera_lifespan.sqchange_tree') #%>% 
  # as.treedata() %>% 
  # full_join(
  #   life.anc,
...
tr.size <- read.tree('../data/stableTraits/UphamEtAl2019_newChiroptera.sqchange_tree')

size.anc <- read_tsv('../data/stableTraits/UphamEtAl2019_newChiroptera.ancstates') %>% 
      filter(!str_starts(Parameter, 'stable')) %>% 
  rename(
    label=Parameter,
...
tr.og.size <- read.tree('../data/stableTraits/UphamEtAl2019_newChiroptera.tree')
...
```{r save RICR results, eval=F, include=F}
tr.cancerSucceptibility %>% 
  treeio::write.beast('../output/Upham2019_newChiroptera-RICR-Data.nexus')
```

```{r load precalc RICR results}
...
```{r RangePlot RICR - bats}
p.RICR.bats <- tr.cancerSucceptibility %>%
  tidytree::tree_subset(MRCA(tr.cancerSucceptibility, 'Myotis_lucifugus', 'Pteropus_vampyrus'), levels_back = 0) %>%
  as_tibble %>% 
  arrange(log2CancerSucceptabilityChange) %>% 
  mutate(label = factor(label, levels=label)) %>% 
...
p.tr.myotis <- tr.cancerSucceptibility %>%
  tidytree::keep.tip(tr.lifedata.chiroptera@phylo$tip.label) %>% 
  tidytree::tree_subset(MRCA(tr.upham %>%
  keep.tip(tr.lifedata.chiroptera@phylo$tip.label), 'Myotis_lucifugus', 'Nycticeius_humeralis'), levels_back = 0) %>% 
  ggtree(
    size=1
...
p.tr.myotis2 <- (tr.cancerSucceptibility %>%
  tidytree::keep.tip(tr.lifedata.chiroptera@phylo$tip.label) %>%
  tidytree::tree_subset(MRCA(tr.upham %>%
  keep.tip(tr.lifedata.chiroptera@phylo$tip.label), 'Myotis_lucifugus', 'Nycticeius_humeralis'), levels_back = 0) %>%
  ggtree(
...
node.vespertilionidae = MRCA(tr.RICR.chiroptera, 'Myotis_lucifugus', 'Corynorhinus_townsendii')
...
p.sizelife.chiroptera <- tr.RICR.chiroptera %>% 
  as_tibble %>% 
  mutate(isTip = sapply(node, function(x){isTip(tr.sizelife.lq.chiroptera , x)})) %>% 
  ggplot(
    aes(
      x= lnSize.median,
      y= lnLife.median,
      color= log2CancerSucceptabilityChange,
      fill=log2CancerSucceptabilityChange,
      alpha = ifelse(isTip, 25, 0)
    )
  ) + 
  geom_point(pch=21) + 
  guides(alpha=guide_none()) + 
  labs(x="Median Estimated Body Size",
       y="Median Estimated Lifespan") + 
  scale_color_viridis_c(guide=guide_none()) + 
  scale_fill_viridis_c() + 
  theme_pubr() + 
  labs_pubr()

p.sizelife <- tr.cancerSucceptibility %>% 
  as_tibble %>% 
  mutate(isTip = sapply(node, function(x){isTip(tr.sizelife.lq , x)})) %>% 
  ggplot(
    aes(
      x= lnSize.median,
      y= lnLife.median,
  ...
  geom_point(data = tr.RICR.chiroptera %>% 
               as_tibble %>% 
               mutate(isTip = sapply(node, function(x){isTip(tr.RICR.chiroptera , x)})), 
             mapping = aes(
               color=log2CancerSucceptabilityChange,
               fill=log2CancerSucceptabilityChange
             ), 
...
(p.sizelife + ggtitle('All Mammals')) + (p.sizelife.chiroptera + ggtitle('Bats'))


p.sizelife2 <- tr.cancerSucceptibility %>% 
  as_tibble %>% 
  mutate(isTip = sapply(node, function(x){isTip(tr.sizelife.lq , x)})) %>% 
  ggplot(
    aes(
  ...
  geom_point(pch=21, aes(color=log2CancerSucceptabilityChange)) +
  geom_point(data = tr.RICR.chiroptera %>% 
               as_tibble %>% 
               mutate(isTip = sapply(node, function(x){isTip(tr.RICR.chiroptera , x)})), 
             mapping = aes(
...
(p.sizelife2 + ggtitle('All Mammals')) + (p.sizelife.chiroptera + ggtitle('Bats'))
```
...
p.layout = c(
  ...
  area(t = 1, l = 3, b = 2, r = 5),
  area(t = 3, l = 3, b = 4, r = 5),
  area(t = 4, l = 1, b = 5, r = 2)
)

(p.tr.myotis) + (p.sizelife2 + ggtitle('All Mammals')+theme(legend.position = 'none')) + (p.sizelife.chiroptera + ggtitle('Bats')) + guide_area() + plot_layout(design = p.layout, guide='collect')
...
p.layout = c(
  area(t = 1, l = 1, b = 6, r = 3), # Phylo
  area(t = 1, l = 3, b = 2, r = 5), # RICR-All
  area(t = 3, l = 3, b = 4, r = 5), # RICR-Bat
  area(t = 5, l = 3, b = 6, r = 5), # Virus - PGLS
  area(t = 2, l = 4, b = 2, r = 5)  # guides
)

(p.tr.myotis) + 
  (p.sizelife2 + ggtitle('All Mammals')+theme(legend.position = 'none')) + 
  (p.sizelife.chiroptera + ggtitle('Bats')) + 
  p.coor + 
  guide_area() +
  plot_layout(design = p.layout, guide='collect')
...
```{r phenograms}
p.phenogram.life <- tr.RICR.chiroptera %>% 
  ggtree(
    aes(color=log2CancerSucceptabilityChange),yscale = "lnLife.median"
  ) + 
  ggrepel::geom_text_repel(aes(label=label %>% str_replace("_", " ") %>% str_remove('\\d+'))) + 
  xlim(c(NA,80)) + 
  scale_color_viridis_c('RICR') + 
  theme_pubclean() + 
  theme(legend.position='bottom') +
  ylab("Lifespan (log)")

p.phenogram.size <- tr.RICR.chiroptera %>% 
  ggtree(
    aes(color=log2CancerSucceptabilityChange),yscale = "lnSize.median"
  ) + 
  ggrepel::geom_text_repel(aes(label=label %>% str_replace("_", " ") %>% str_remove('\\d+'))) + 
  xlim(c(NA,80)) + 
  scale_color_viridis_c('RICR') + 
...
# p.phenogram.lifeVsSize <- tr.RICR.chiroptera %>% 
#   ggtree(
#     aes(color=log2CancerSucceptabilityChange),yscale = "lnLife.median", branch.length = 'lnSize.median'
#   ) + 
#   geom_point(
#     data= tr.RICR.chiroptera %>% as_tibble(),
#     mapping = aes(x=lnSize.median, y=lnLife.median, color=log2CancerSucceptabilityChange)
#   ) + 
#   scale_color_viridis_c('RICR') + 
#   theme_pubclean() + 
#   theme(legend.position='bottom') +
...
tbl.RICR.chiroptera <- tr.RICR.chiroptera %>% as_tibble

tbl.RICR.chain <- tbl.RICR.chiroptera %>% 
  select(parent, node, K1)

RICR.luc <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_lucifugus') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.3=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.4=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.5=K1), by=c('parent'='node')) %>% 
  select(-parent, -parent.y, -node)
  
RICR.occ <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_occultus') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
...
RICR.vel <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_velifer') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
...
RICR.yum <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_yumanensis') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
...
RICR.evo <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_evotis') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
  ...
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.4=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.5=K1), by=c('parent'='node'))%>%
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.6=K1), by=c('parent'='node'))%>%
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.7=K1), by=c('parent'='node'))%>%
  select(-parent) %>% rename(parent=parent.y) %>% left_join(tbl.RICR.chain %>% rename(K1.8=K1), by=c('parent'='node')) %>%
 select(-parent, -parent.y, -node)

RICR.thy <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_thysanodes') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
...
RICR.cal <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_californicus') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
...
RICR.vol <- tbl.RICR.chiroptera %>% 
  filter(label == 'Myotis_volans') %>% 
  select(parent, node, label, K1) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.1=K1), by=c('parent'='node')) %>% 
  select(-parent) %>% rename(parent=parent.y) %>% 
  left_join(tbl.RICR.chain %>% rename(K1.2=K1), by=c('parent'='node')) %>% 
...
## Nodes of Note from RICR
```{r}
# tbl.RICR.chiroptera %>% colnames()

# 1657 is Luc-Occ
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1657) %>% pull(node), type="tips"))

# 1646 is the root of Myotis
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1646) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange)

# 1648 is long-lived EuroMyo, noBrandt
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1648) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)

# 1648 is long-lived EuroMyo, yesBrandt
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1647) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)

# 1630 is Vespertilliodinae
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1630) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)

# 1653 is volans, sodalis, mystacinus
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1653) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)

# 1662 is Evo-Thy-Aui-Cai
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1662) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)

# 1663 is Sept-Auri
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1663) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)

# 1654 is volans, mystacinus
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1654) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)

# 1649 is EuroMyo not myo not brandt
tbl.RICR.chiroptera %>% filter(node %in% offspring(tr.RICR.chiroptera, tbl.RICR.chiroptera %>% filter(label==1649) %>% pull(node), type="tips")) %>% select(label, log2CancerSucceptabilityChange) %>% print(n=121)
...
```{r}
tbl.RICR.bats.withSummary <- tr.RICR.chiroptera %>% as_tibble()%>% 
    select(label, lnSize.median, lnLife.median, lnSize.low, lnLife.low, lnSize.high, lnLife.high, lnK1, lnK2, log2CancerSucceptabilityChange) %>% 
    bind_rows(tr.RICR.summary) %>% 
  arrange(desc(log2CancerSucceptabilityChange)) %>% 
  mutate(
    label_clean = sapply(label, function(x){label_clean[x]}),
    label= factor(label, levels=label)
  )


p.RICR.myotis.sumv2 <- tbl.RICR.bats.withSummary %>% 
  ggplot(
    aes(
      y=label,
      x=lnK1
  ...
  geom_bar(
    data = . %>% filter(label %in% names(highlight_tips_nodes)),
    aes(
      x=Inf,
      fill=label
    ),
  ...
  labs_pubr() + 
  scale_y_discrete(
    breaks = names(highlight_tips_nodes),
    labels = label_clean[names(highlight_tips_nodes)]
  ) + 
  scale_color_viridis_c(
    option = 'D',
    guide = guide_colorbar(
...
```{r}
p.RICR.myotis.sumv3 <- tbl.RICR.bats.withSummary %>% 
  ggplot(
    aes(
      y=label,
      x=log2CancerSucceptabilityChange
  ...
  geom_point(aes(color=log2CancerSucceptabilityChange), size=1) +
  geom_text_repel(
    data=tbl.RICR.bats.withSummary %>% filter(!is.na(label_clean), log2CancerSucceptabilityChange>0),
    aes(
      # x=-11,
      label=label_clean
    ),
    ...
    direction = 'y',
    # nudge_x = -1,
    nudge_x = 13-tbl.RICR.bats.withSummary %>% filter(!is.na(label_clean), log2CancerSucceptabilityChange>0) %>% pull(log2CancerSucceptabilityChange),
  ...
  expand_limits(
    # x = c(-13,13),
    y = c(
      tbl.RICR.bats.withSummary$label %>% as.numeric() %>% min() %>% subtract(5),
      tbl.RICR.bats.withSummary$label %>% as.numeric() %>% max() %>% add(5)
    )
  ) + 
  scale_color_viridis_c(
    option = 'D',
...
```{r}
p.RICR.myotis.sumv3.2 <- tbl.RICR.bats.withSummary %>% 
  ggplot(
    aes(
      y=label,
      x=log2CancerSucceptabilityChange
  ...
  annotate(
    geom='text', 
    x= -7,
    y= tbl.RICR.bats.withSummary$label[tbl.RICR.bats.withSummary$label=='Myotis_auriculus'],
  ...
  annotate(
    geom='text', 
    x= 7,
    y= tbl.RICR.bats.withSummary$label[tbl.RICR.bats.withSummary$label=='Myotis_grisescens'],
...
p.RICR.myotis.sumv3.2
```

```{r}
tbl.RICR.withBatSummary <- tr.cancerSucceptibility %>% as_tibble()%>% 
    select(label, lnSize.median, lnLife.median, lnSize.low, lnLife.low, lnSize.high, lnLife.high, lnK1, lnK2, log2CancerSucceptabilityChange) %>% 
    bind_rows(tr.RICR.summary) %>% 
  arrange(desc(log2CancerSucceptabilityChange)) %>% 
  mutate(
...
min.RICR.lab = tbl.RICR.withBatSummary %>% filter(log2CancerSucceptabilityChange==min(log2CancerSucceptabilityChange)) %>% pull(label)
max.RICR.lab = tbl.RICR.withBatSummary %>% filter(log2CancerSucceptabilityChange==max(log2CancerSucceptabilityChange)) %>% pull(label)

p.RICR.myotis.sumv4 <- tbl.RICR.withBatSummary %>% 
  ggplot(
    aes(
      y=label,
      x=log2CancerSucceptabilityChange
  ...
  geom_point(aes(color=log2CancerSucceptabilityChange), size=1) +
  geom_text_repel(
    data=tbl.RICR.withBatSummary %>% filter(!is.na(label_clean), log2CancerSucceptabilityChange>0),
    aes(
      # x=-11,
      label=label_clean
    ),
    ...
    direction = 'y',
    # nudge_x = -1,
    nudge_x = 13-tbl.RICR.withBatSummary %>% filter(!is.na(label_clean), log2CancerSucceptabilityChange>0) %>% pull(log2CancerSucceptabilityChange),
    min.segment.length = 0,
    segment.curvature = -1e-20
  ) + 
  geom_text_repel(
...
p.RICR.myotis.sumv4.2 <- tbl.RICR.withBatSummary %>% 
  ggplot(
    aes(
      y=label,
      x=log2CancerSucceptabilityChange
  ...
  geom_text_repel(
    ...
    hjust=1,
    direction = 'y',
    nudge_x = 15-tbl.RICR.withBatSummary %>% filter(!is.na(label_clean), log2CancerSucceptabilityChange>0) %>% pull(log2CancerSucceptabilityChange),
    min.segment.length = 0,
    segment.curvature = -1e-20
  ) + 
  geom_text_repel(
