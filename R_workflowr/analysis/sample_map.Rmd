...
shape.myoAui <- st_read("../data/USGS_SpeciesRanges/Myotis_auriculus/mSWMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_auriculus") %>% st_make_valid()
shape.myoCai <- st_read("../data/USGS_SpeciesRanges/Myotis_californicus/mCAMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_californicus") %>% st_make_valid()
shape.myoOcc <- st_read("../data/USGS_SpeciesRanges/Myotis_occultus/mARMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_occultus") %>% st_make_valid()
shape.myoLuc <- st_read("../data/USGS_SpeciesRanges/Myotis_lucifugus/mLBMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_lucifugus") %>% st_make_valid()
shape.myoYum <- st_read("../data/USGS_SpeciesRanges/Myotis_yumanensis/mYUMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_yumanensis") %>% st_make_valid()
shape.myoVol <- st_read("../data/USGS_SpeciesRanges/Myotis_volans/mLLMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_volans") %>% st_make_valid()
shape.myoVel <- st_read("../data/USGS_SpeciesRanges/Myotis_velifer/mCVMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_velifer") %>% st_make_valid()
shape.myoEvo <- st_read("../data/USGS_SpeciesRanges/Myotis_evotis/mLEMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_evotis") %>% st_make_valid()
shape.myoThy <- st_read("../data/USGS_SpeciesRanges/Myotis_thysanodes/mFRMYx_CONUS_Range_2001v1.shp") %>% mutate(species = "Myotis_thysanodes") %>% st_make_valid()
```
...
notwest <- west <- subset(states, !(ID %in% c("california", "arizona", "nevada", "new mexico", "idaho", "oregon", "washington", "montana", "nebraska", "colorado", "utah", "wyoming")))
...
p.map.collections <- ggplot() + 
  ...
  geom_spatial_label_repel(data= datasheet.known, aes(label=species,group=species, y=Lattitude, x=Longitude, color=species)) + 
...
```{r [old] MVZ plot world, eval=F, include=F}
species.seq %>% 
  dplyr::select(SPECIES, latitude, longitude) %>% 
  {tibble(type = c("min", "max"), latitude=c(min(.$latitude, na.rm = T), max(.$latitude, na.rm = T)), 
          longitude = c(min(.$longitude, na.rm = T), max(.$longitude, na.rm = T)))}
plot.dist.point <- 
  ggplot(data = cal) +
  geom_sf(data=cal)+
  geom_point(data = species.seq,
             mapping=aes(x=longitude, y=latitude, color=SPECIES, shape=HQ), inherit.aes = F) +
  coord_sf(xlim = c(-124, -114.4), ylim=c(32, 42.1)) + 
  ggsci::scale_color_d3("Species", palette ="category20", alpha=0.3) + 
  theme_pubr() + 
  theme(legend.position = "right")
plot.dist.point %>% plotly::ggplotly()
```

```{r [old] MVZ Time plot, eval=F, include=F}
species.seq %>% 
  ggplot(
    aes(
      x= YEAR_COLLECTED,
  ...
  geom_histogram() +
  geom_histogram(data=species.seq %>% filter(HQ), color="black") + 
  ggsci::scale_color_d3("Species", palette ="category20", alpha=0.3) + 
  ggsci::scale_fill_d3("Species", palette ="category20", alpha=0.3) + 
