...
```{r}
phy <- readMCMCtree('../data/timetree/Run2_real_combined_allGenesAllSpecies/FigTree.tre')

phy.onlyMyotis <- phy$apePhy %>% extract.clade(MRCA(phy$apePhy, 'Myotis_lucifugus', 'Myotis_davidii'))
...
trdat.phy.onlyMyotis <- trdat.phy %>% tidytree::drop.tip(trdat.phy@phylo$tip.label[str_detect(trdat.phy@phylo$tip.label, "Myotis", negate=T)])
...
MCMC.tree.plot(phy, MCMC.chain = read_tsv('../data/timetree/Run2_real_combined_allGenesAllSpecies/Run2_real_combined124.mcmc'), cex.tips = 0.2,
...
MCMC.tree.plot(test, cex.tips = 0.6, time.correction = 100, scale.res = c("Eon",
